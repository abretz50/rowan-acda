...
// itself restricted to E-Board accounts. `parentId` makes a comment a reply
// to another (top-level only — replies don't themselves get a Reply button).
async function withPhotos(comments) {
  const members = await loadMembers();
  const photoById = new Map(members.map(m => [m.id, m.photoUrl || null]));
  return comments.map(c => ({ ...c, photoUrl: photoById.get(c.memberId) || null }));
}
...
export default async function handler(req) {
  ...
  if (req.method === 'GET') {
    ...
    if (eventId) {
      return json({ ok: true, comments: await withPhotos(comments.filter(c => c.eventId === eventId)) });
    }
    ...
    const recent = (await withPhotos(comments))
  ...
  if (req.method === 'POST') {
    ...
    const comments = await getCollection('eventComments', []);
    if (parentId && !comments.some(c => c.id === parentId && c.eventId === eventId)) {
      return json({ ok: false, error: 'Comment being replied to was not found.' }, 404);
    }
    const comment = {
      id: randomUUID(), eventId, parentId: parentId || null, memberId: auth.user.id, memberName: auth.user.name,
      text: text.trim().slice(0, 1000), createdAt: new Date().toISOString(),
    };
    comments.push(comment);
    await setCollection('eventComments', comments);
    return json({ ok: true, comment: (await withPhotos([comment]))[0] });
  }
  ...
  if (req.method === 'DELETE') {
    ...
    // moderate anyone else's. Deleting a comment also removes its replies.
    if (target.memberId !== auth.user.id && auth.user.role === 'member') {
      return json({ ok: false, error: 'Not authorized.' }, 403);
    }
    await setCollection('eventComments', comments.filter(c => c.id !== body.id && c.parentId !== body.id));
    return json({ ok: true });
  }
