...
// concurrent write is detected. Returns null on success, or
// { message, status } on a validation failure.
function applyVolunteerOp(points, event, me, kind, body, perSlot, perItem) {
  ...
  if (kind === 'slot' || kind === 'unsign-slot') {
    if (event.volunteerType !== 'bake_sale' && event.volunteerType !== 'time_slot') {
      return { message: 'This event does not use time slots.', status: 400 };
    }
    if (!slotLabel) return { message: 'slotLabel is required.', status: 400 };
    if (!generateSlots(event).some(s => s.label === slotLabel)) return { message: 'Invalid slot.', status: 400 };

    let mine = findMine();
    if (kind === 'slot') {
      if (mine && (mine.slotLabels || []).includes(slotLabel)) return null; // already signed up
      ...
      if (taken >= capacity) return { message: 'That slot is full.', status: 409 };
      if (mine) {
        if (mine.status === 'approved') return { message: 'Your signup was already approved — ask the secretary to add more.', status: 409 };
      ...
      if (!mine || !(mine.slotLabels || []).includes(slotLabel)) return { message: 'You are not signed up for that slot.', status: 404 };
      if (mine.status === 'approved') return { message: 'This was already approved — ask the secretary to remove it.', status: 409 };
      mine.slotLabels = mine.slotLabels.filter(l => l !== slotLabel);
    }
  } else if (kind === 'items' || kind === 'unsign-items') {
    if (event.volunteerType !== 'bake_sale') return { message: 'This event does not take baked item signups.', status: 400 };
    let mine = findMine();
    if (kind === 'items') {
      const count = Math.round(Number(itemCount));
      if (!count || count < 1 || count > MAX_BAKE_SALE_ITEMS) {
        return { message: `Enter between 1 and ${MAX_BAKE_SALE_ITEMS} items.`, status: 400 };
      }
      const description = String(items || '').trim();
      if (!description) return { message: 'Say what you\'re bringing (e.g. "2 dozen cookies, 1 tray of brownies").', status: 400 };
      if (mine) {
        if (mine.status === 'approved') return { message: 'Your item signup was already approved — ask the secretary to adjust it if it changed.', status: 409 };
      ...
      if (!mine || !mine.itemCount) return { message: 'You do not have an item signup for this event.', status: 404 };
      if (mine.status === 'approved') return { message: 'This was already approved — ask the secretary to remove it.', status: 409 };
      mine.itemCount = 0;
      mine.items = '';
    }
  } else if (kind === 'full') {
    if (event.volunteerType !== 'full_event') return { message: 'This event does not use full-day signup.', status: 400 };
    if (points.some(p => p.eventId === eventId && p.memberId === me.id && p.source === 'volunteer-full' && p.status !== 'denied')) return null;
    ...
    if (event.fullDayCapacity && activeCount >= event.fullDayCapacity) {
      return { message: 'This event is already full.', status: 409 };
    }
    points.push({
      id: randomUUID(), memberId: me.id, memberName: me.name, memberEmail: me.email,
    ...
    return { message: 'Unknown kind.', status: 400 };
  }
...
export default async function handler(req) {
  ...
  await updateCollection('points', [], async (stored) => {
    opError = applyVolunteerOp(stored, event, me, kind, body, perSlot, perItem);
    return stored;
  });
